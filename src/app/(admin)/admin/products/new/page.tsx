	...
	return (
		<main className="min-h-screen bg-black text-white px-6 md:px-10 xl:px-20 py-12">
			...
			<form onSubmit={onSubmit} className="grid md:grid-cols-2 gap-6">
				<input value={form.slug} onChange={(e) => setForm({ ...form, slug: slugify(e.target.value) })} name="slug" placeholder="slug" className="bg-black border border-white/20 px-4 py-3" />
				<input value={form.name} onChange={(e) => onNameChange(e.target.value)} name="name" placeholder="name" className="bg-black border border-white/20 px-4 py-3" />
				<input value={form.priceCents} onChange={(e) => setForm({ ...form, priceCents: Number(e.target.value) })} name="priceCents" placeholder="price" type="number" className="bg-black border border-white/20 px-4 py-3" />
